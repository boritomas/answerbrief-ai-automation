    ...
    <main style={styles.page}>
      ...
      <section style={styles.hero}>
        <div style={styles.heroMain}>
          ...
          <p style={styles.lead}>
            Since the latest verified update, Career OS retained {opportunities.length} strong opportunities,
            submitted {todaySubmitted} application{todaySubmitted === 1 ? '' : 's'} today, and is tracking {waiting} employer response{waiting === 1 ? '' : 's'}.
          </p>
          <div style={styles.notice}>
            <strong>{noAction ? 'No action is required from you right now.' : `${blocked} item${blocked === 1 ? '' : 's'} require your attention.`}</strong>
            <span>{nextRun ? `Next autonomous run: ${nextRun}` : 'The next run will appear after the scheduler reports it.'}</span>
          </div>
        </div>
      ...
      <section id="applications" style={styles.section}>
        ...
        <div style={styles.timeline}>
          ...
          {!activity.length ? <p style={styles.empty}>No persisted application activity is available yet.</p> : null}
        </div>
      </section>
    </main>
  );
...
function whyFit(text: string) {
  ...
  return 'The persisted role cleared Career OS fit and quality thresholds.';
}
...
const styles: Record<string, CSSProperties> = {
  ...
  hero: { maxWidth: 1320, margin: '0 auto', padding: '48px 32px 24px', display: 'grid', gridTemplateColumns: 'minmax(0, 2fr) minmax(300px, .85fr)', gap: 24 },
  ...
  timeline: { display: 'grid', gap: 12 }, timelineItem: { display: 'flex', justifyContent: 'space-between', gap: 24, padding: 22, background: '#fff', border: '1px solid #dce5f0', borderRadius: 16 }, timelineState: { color: '#0d6f5a', fontWeight: 800, fontSize: 12, textTransform: 'uppercase' }, timelineTitle: { margin: '7px 0', fontSize: 18 }, timelineCopy: { margin: 0, color: '#526a87' }, time: { color: '#75869c', whiteSpace: 'nowrap' }, empty: { padding: 24, background: '#fff', borderRadius: 16 },
};
