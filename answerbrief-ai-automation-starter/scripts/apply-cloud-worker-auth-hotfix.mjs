...
if (!source.includes(oidcAuth)) {
  if (source.includes(fallbackSharedAuth)) {
    source = source.replace(fallbackSharedAuth, oidcAuth);
    changed = true;
  } else if (source.includes(originalSharedAuth)) {
    source = source.replace(originalSharedAuth, oidcAuth);
    changed = true;
  }
}

const legacyPredicate = 'find((key): key is JsonWebKey => key.kid === header.kid)';
const safeLookup = 'find((key) => key.kid === header.kid)';
if (source.includes(legacyPredicate)) {
  source = source.replace(legacyPredicate, safeLookup);
  changed = true;
}

const legacySigningInput = "Buffer.from(`${parts[0]}.${parts[1]}`)";
const safeSigningInput = "Buffer.from(parts[0] + '.' + parts[1])";
if (source.includes(legacySigningInput)) {
  source = source.replace(legacySigningInput, safeSigningInput);
  changed = true;
}

if (changed) fs.writeFileSync(workerFile, source);
