...
function orderToRow(order: Order): SupabaseOrderRow {
  return {
    ...
    delivery_status: order.deliveryStatus,
    drive_error: order.driveError ?? null,
    drive_folder_id: order.driveFolderId ?? null,
    drive_folder_url: order.driveFolderUrl ?? null,
    error_message: order.errorMessage ?? null,
    generated_brief_mode: order.generatedBriefMode ?? null,
    generated_brief_url: order.generatedBriefUrl ?? null,
    id: order.id,
    intake: order.intake ?? null,
    intake_status: order.intakeStatus,
    intake_submitted_at: order.intakeSubmittedAt ?? null,
    intake_token_hash: order.intakeTokenHash ?? null,
    logs: order.logs ?? [],
    package_key: order.packageKey ?? null,
    package_name: order.packageName,
    payment_status: order.paymentStatus,
    prep_workspace_url: order.prepWorkspaceUrl ?? null,
    status: order.status,
